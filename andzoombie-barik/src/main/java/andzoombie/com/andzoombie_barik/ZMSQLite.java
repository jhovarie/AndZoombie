...
public class ZMSQLite extends SQLiteOpenHelper {
    private static final String TAG = "ZMSQLite";
    ...
    public ZMSQLite(Context context,String dbname) {
    ...
    public void updateTableWhereID(String table, String columntoupdate, String newvalue, String wherecolumn,int wherevalue){
        SQLiteDatabase db = getWritableDatabase();
        db.execSQL("UPDATE "+table+ " set "+columntoupdate+" = \""+newvalue+"\" WHERE "+ wherecolumn + "= " + wherevalue + ";");
        Log.d(TAG,"updateTable");
    }

    public void updateTableWhereID(String table, String columntoupdate1, String newvalue1,String columntoupdate2, String newvalue2, String whereid,int wherevalue){
        SQLiteDatabase db = getWritableDatabase();
        db.execSQL("UPDATE "+table+ " set "+columntoupdate1+" = \""+newvalue1+"\",  "+columntoupdate2+" = \""+newvalue2+"\"  WHERE "+ whereid + "=" + wherevalue + ";");
        Log.d(TAG,"updateTable");
    }
    */


    public  void updateTableWhere(String table,String cols[],String typs[], String wherecolumn, String wherevalue) {
        ...
        q+=" WHERE "+wherecolumn+" = \""+wherevalue+"\";";
        SQLiteDatabase db = getWritableDatabase();
        db.execSQL(q);
    }

    public  void updateTableWhereID(String table,String cols[],String typs[], String columid, int id) {
        String q = "UPDATE "+table+" set ";
        for(int i = 0; i < cols.length; i++) {
            q += cols[i] + "=\""+typs[i]+"\"";
            if(i + 1 < cols.length) {
    ...
    public void deleteRowID(String table, String idstr, int idval) {
        SQLiteDatabase db = getWritableDatabase();
        db.execSQL("DELETE FROM " + table + " WHERE " + idstr + "=" + idval + ";");
        Log.d(TAG,"deleteProduct");
    }
    ...
    public String[] selectRowWhereEqual(String table, String column, String value, String returncolumn[]){
        ...
        while (!c.isAfterLast()) {
            if (c.getString(c.getColumnIndex(column)) != null) {
                ...
                //str.addnotes(c.getString(c.getColumnIndex(column)));
                //dbString += "\n";
            }
            c.moveToNext();
        }
